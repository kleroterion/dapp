...
class Kleroterion extends Component {
  ...
  getDisputes = () => {
    ...
    .nbDispute({from: web3.eth.accounts[0]}, (res,err) => {
      ...
      let disputesArr = [...new Array(err.toNumber()).keys()]
      disputesArr.map(
        i => this.state.buyableCourtContract.at('0xc92aa8aF07aD57d023E8A2FE18175D69dDd6b02f').disputes(i, {from: web3.eth.accounts[0]}, (res,err) => {
          this.setState({disputes: [...this.state.disputes, err]})
          console.log(err)
        })
      )
    })
